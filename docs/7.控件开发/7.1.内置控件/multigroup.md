# multigroup 组织多选框
组织多选控件,该控件用于同时选择多个组织部门;

## 效果展示

![](../../assets/gitbook/7.控件开发/7.1.内置控件/7.2.multigroup1.png)<br>点击选择可得下图

![](../../assets/gitbook/7.控件开发/7.1.内置控件/7.2.multigroup2.png)
...
|change |1 |必选 |回调函数,入参$this是控件对应的jquery对象. |设置控件事件回调函数.控件触发blur时调用.<br>调用示例:<br>Widget.change($form,name,function($this){<br>alert($this.val());<br>});|
    ...
    说明:支持两种参数
    1.传入一个val的map集合就是默认一个默认值;
    2.传入一个root的map集合就是默认一个组织根节点;
    3.也支持在页面调用init()方法来进行初始化调用;

## 示例1:选择多个组织
![](../../assets/gitbook/7.控件开发/7.1.内置控件/7.2.multigroup4.png)
